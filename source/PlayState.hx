...
class PlayState extends MusicBeatState
{
	...
	var rockbgbounce:FlxSprite;
	var isPostDialogue:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'cave'://sadrock :spingebill:
				rockbgbounce = new FlxSprite(-500, -150);
        	    rockbgbounce.frames = Paths.getSparrowAtlas('cave/RECOVER_sad_rock_bg');
                rockbgbounce.animation.addByPrefix('bounce', 'Bounce6', 24, true);
                rockbgbounce.scrollFactor.set(1.0, 1.0);
                rockbgbounce.antialiasing = true;
                add(rockbgbounce);
	...
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if(dialogueFile.dialogue.length > 0) {
			...
			doof.scrollFactor.set();
			if(isPostDialogue == true) {
				doof.finishThing = postSongCutscene;
			} else {
				doof.finishThing = startCountdown;
			}
			doof.nextDialogueThing = startNextDialogue;
	...
	function finishSong():Void
	{
		...
		if(isStoryMode && Paths.formatToSongPath(SONG.song) == 'sad')
			{
				finishCallback = postSongDialogue;
			}
	...
	function postSongCutscene() 
	{
		startVideo("spunchbop");
	}
	...
	function postSongDialogue() 
	{
		...
		var songName:String = Paths.formatToSongPath(SONG.song);
		var file:String = Paths.json(songName + '/dialogueEnd');
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'cave':
				if(!ClientPrefs.lowQuality) {
				rockbgbounce.animation.play('bounce', true);
				}
